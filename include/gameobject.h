...
public:
	...
	void AddComponent(Args&&... args) {
		static_assert(std::is_base_of<Component, T>::value, "In: GameObject::AddComponent -> T must inherit from Component");
		components.push_back(std::make_unique<T>(std::forward<Args>(args)...));
	}

	template<typename T>
	bool HasComponent() const {
		static_assert(std::is_base_of<Component, T>::value, "In: GameObject::HasComponent -> T must inherit from Component");
	...
	T* GetComponent() const {
		static_assert(std::is_base_of<Component, T>::value, "In: GameObject::GetComponent -> T must inherit from Component");
		for (const auto& comp : components) {
	...
	std::vector<T*> GetComponents() const {
		static_assert(std::is_base_of<Component, T>::value, "In: GameObject::GetComponents -> T must inherit from Component");
...
public:
	...
	void AddAnimation(const std::string& name, const AnimationComponent& animation);
